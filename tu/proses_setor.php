...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $jumlah_setor = isset($_POST['jumlah_setor']) ? (float) $_POST['jumlah_setor'] : 0.0;
    $keterangan = isset($_POST['keterangan']) ? trim($_POST['keterangan']) : '';

    // Validasi data
    if ($id_siswa <= 0 || $jumlah_setor <= 0) {
        echo "<script>alert('ID Siswa atau jumlah setor tidak valid.'); window.location.href='tabungan.php';</script>";
        exit;
    }
    ...
    // Hitung saldo baru
    $saldo_baru = $saldo_sebelumnya + $jumlah_setor;

    // Masukkan data setoran baru ke dalam tabel tabungan_siswa
    ...
    $stmt->bind_param("idds", $id_siswa, $jumlah_setor, $saldo_baru, $keterangan);

    // Eksekusi dan cek apakah data berhasil disimpan
    if ($stmt->execute()) {
        echo "<script>alert('Setoran berhasil ditambahkan.'); window.location.href='tabungan.php';</script>";
    } else {
        echo "<script>alert('Gagal menambahkan setoran.'); window.location.href='tabungan.php';</script>";
    }
